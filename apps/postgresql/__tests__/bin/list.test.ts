...
describe('list', () => {
  ...
  describe('when valid S3 options are specified', () => {
    ...
    beforeEach(async() => {
      await uploadFixtureToTestS3Bucket('backup-20220402000000.tar.bz2');
    });

    it('list files in bucket', async() => {
      expect(await exec(commandLine)).toEqual({
        stdout: expect.stringContaining('backup-20220402000000.tar.bz2'),
        stderr: '',
      });
    });
  });
  ...
  describe('when valid GCS options are specified', () => {
    ...
    beforeEach(async() => {
      await uploadFixtureToTestGCSBucket('backup-20220402000000.tar.bz2');
    });
