...
1. Normal mode (using temporary files)
   ...
   - The backup file name is generated by `getBackupFileName()` and is cached (the same options always return the same name in one execution). The file extension is determined by each command via `getBackupFileExtension()`.
   - Implementation example:
     ...
     async backupOnce(options: IBackupCommandOption): Promise<void> {
       ...
       await this.storageServiceClient.copyFile(dbDumpFilePath, options.targetBucketUrl.toString(), backupFileName);
     }
     ```
...
2. Streaming mode (without using temporary files)
   ...
   - The backup file name is generated by `getBackupFileName()`.
   - Implementation example:
     ```typescript
